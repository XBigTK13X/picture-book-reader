...
import static com.simplepathstudios.pbr.api.model.MusicQueue.UpdateReason.PLAYER_STATE_CHANGED;
...
public class ObservableMusicQueue {
    ...
    public void addItems(ArrayList<MusicFile> items){
        ...
        for(MusicFile item : items){
            if(item.AudioDuration != null && item.AudioDuration <= PBRSettings.SongDurationMinimumSeconds){
                foundCount += 1;
                continue;
            }
