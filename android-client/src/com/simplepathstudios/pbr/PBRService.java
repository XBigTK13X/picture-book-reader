...
public class PBRService extends MediaBrowserService {

    public static android.content.ComponentName ComponentName = new ComponentName("com.simplepathstudios.pbr.PBRService", PBRService.class.getName());

    public static class PBRBroadcastReceiver extends BroadcastReceiver{
    ...
    private static final String TAG = "PBRService";
    private static final String WAKE_LOCK_TAG = "pbr:background_audio";

    public static PBRService __instance;
    public static PBRService getInstance(){
        return __instance;
    }
    ...
    PBRBroadcastReceiver broadcastReceiver;
    ...
    public MediaSession getMediaSession(){
        if(mediaSession == null || mediaSession.getSessionToken() == null){
            ...
            mediaSession = new MediaSession(Util.getGlobalContext(), "PBRMediaSession");
    ...
    public void onCreate() {
        ...
        ObservableCastContext.getInstance().observe(castContext->{
            ...
            this.castContext.addCastStateListener(castState -> {
                if (castState == CastState.NOT_CONNECTED) { // A user killed the session from within PBR
                ...
                } else if(castState == CastState.NO_DEVICES_AVAILABLE){ // The session died outside of PBR
                    Util.log(TAG, "Cast session changed state to " + CastState.toString(castState) + " use the local player");
                    if(audioPlayer != null){
        ...
        broadcastReceiver = new PBRBroadcastReceiver();
    ...
    public void onDestroy() {
        Util.log(TAG, "Something destroyed the pbr service");
        super.onDestroy();
        cleanup();
    }
